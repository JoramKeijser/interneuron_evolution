...
for (cell_type in c( "GABA", "Glut")){
  print(paste("Comparing cell type", cell_type))
  ...
  print("Find marker genes: bird")
  top.bird.markers <- get_top_markers(bird) 
  print("Find marker genes: mouse")
  ...
  if (cell_type == "GABA"){
    # Throw out "subcortical" songbird cells. 
    # !rownames(C) %in% c("Serpinf1", "Sncg")
    C <- C[, !colnames(C) %in% c("Pre", "7", "8")]
    # Reorder
    C <- C[c("Meis2", "Sst", "Pvalb", "Vip", "Lamp5", "Serpinf1", "Sncg"),]
    }
  else if (cell_type == "Glut"){
  ...
  fname <- paste0("./figures/corr_", cell_type, ".pdf")
  print(paste("Visualize & save correlation matrix as", fname))
  fontsize <- 25
